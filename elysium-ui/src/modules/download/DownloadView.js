...
export const downloadModule = {
    ...
    render() {
        ...
        vp.innerHTML = `
            <h2 class="view-title" data-i18n="dl_title">${t('dl_title')}</h2>
            <p style="color:var(--text-muted); font-size:0.95rem; margin-bottom:24px;" data-i18n="dl_sub">${t('dl_sub')}</p>
            <div style="display:flex; gap:12px; margin-bottom:32px;">
                <input type="text" id="download-input" data-i18n="dl_placeholder" placeholder="${t('dl_placeholder')}" style="flex:1; padding:12px 16px; background:var(--bg-sidebar); border:1px solid var(--border-subtle); border-radius:6px; color:var(--text-main); font-size:0.95rem; outline:none;">
                <button id="download-trigger" data-i18n="dl_btn" style="background:var(--accent-premium); border:none; color:white; font-weight:600; padding:0 24px; border-radius:6px; cursor:pointer; font-size:0.95rem;">${t('dl_btn')}</button>
            </div>
            <div style="padding:24px; border:1px solid var(--border-subtle); background:var(--bg-sidebar); border-radius:8px; margin-bottom:24px;">
                <h3 style="font-size:1rem; margin-bottom:8px; color:var(--text-main); font-weight:600;" data-i18n="import_title">${t('import_title')}</h3>
                <p style="color:var(--text-muted); font-size:0.85rem; margin-bottom:16px;" data-i18n="import_sub">${t('import_sub')}</p>
                <button id="import-trigger" data-i18n="import_btn" style="background:rgba(255,255,255,0.03); border:1px solid var(--border-subtle); color:var(--text-main); font-weight:600; padding:10px 18px; border-radius:6px; cursor:pointer; font-size:0.85rem; transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.08)'" onmouseout="this.style.background='rgba(255,255,255,0.03)'">${t('import_btn')}</button>
