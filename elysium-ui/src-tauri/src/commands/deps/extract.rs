...
pub fn extract_zip(zip_path: &str, dest_dir: &str) -> Result<(), String> {
    println!("[Deps:Extract] Extracting zip: {} -> {}", zip_path, dest_dir);
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .output()
        .map_err(|e| {
            let msg = format!("PowerShell extract failed: {}", e);
            println!("[Deps:Extract] ERROR: {}", msg);
            msg
        })?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        let msg = format!("Extract failed: {}", stderr.trim());
        println!("[Deps:Extract] ERROR: {}", msg);
        return Err(msg);
    }
    println!("[Deps:Extract] Zip extraction complete");
    Ok(())
}
...
pub fn extract_tar_xz(tar_path: &str, dest_dir: &str) -> Result<(), String> {
    println!("[Deps:Extract] Extracting tar.xz: {} -> {}", tar_path, dest_dir);
    let output = std::process::Command::new("tar")
        .args(["-xf", tar_path, "-C", dest_dir, "--strip-components=1"])
        .creation_flags(CREATE_NO_WINDOW)
        .output()
        .map_err(|e| {
            let msg = format!("tar extract failed: {}", e);
            println!("[Deps:Extract] ERROR: {}", msg);
            msg
        })?;
    ...
    println!("[Deps:Extract] tar.xz extraction complete");
    Ok(())
}
...
pub fn collect_binaries(dest_dir: &str) -> Result<Vec<String>, String> {
    println!("[Deps:Extract] Collecting binaries from: {}", dest_dir);
    ...
    if dest.join(ffmpeg_name).exists() && dest.join(ffprobe_name).exists() {
        ...
        println!("[Deps:Extract] Binaries already at root level: {:?}", installed);
        return Ok(installed);
    }
        ...
        for entry in entries.flatten() {
            ...
            if let Ok(sub_entries) = std::fs::read_dir(&path) {
                for sub in sub_entries.flatten() {
                    ...
                    if sub_bin.exists() {
                        println!("[Deps:Extract] Found nested bin/ dir: {}", sub_bin.display());
                        copy_if_missing(&sub_bin, dest, ffmpeg_name, &mut installed);
                        copy_if_missing(&sub_bin, dest, ffprobe_name, &mut installed);
                        if !installed.is_empty() {
                            println!("[Deps:Extract] Found binaries in: {}", sub_bin.display());
                            return Ok(installed);
                        }
                    }
                }
    ...
    if installed.is_empty() {
        println!("[Deps:Extract] WARNING: No ffmpeg/ffprobe binaries found in archive");
...
fn copy_if_missing(src_dir: &Path, dest_dir: &Path, name: &str, out: &mut Vec<String>) {
    ...
    if src.exists() && !dst.exists() {
        println!("[Deps:Extract] Copying: {} -> {}", src.display(), dst.display());
